...
// –– Config ––
const FLOW_URL = “https://prod-58.usgovtexas.logic.azure.us:443/workflows/” +
“ceefc1e6e256421c9a5a83416ff6c167/triggers/manual/paths/invoke?” +
“api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&” +
“sig=lh1qRrhhVYUZ3ZhxuPpHYJPsdhdMmZDRSDndshkjiB0”;
...
if (emp.position && POSITION_GLS[emp.position]) {
  ...
  if (!choices.some(c => c.value === gl)) {
    choices.push({ 
      label: `Position (${emp.position}) — ${gl}`, 
      value: gl 
    });
  }
}
...
if (deptGL && !choices.some(c => c.value === deptGL)) {
  choices.push({ 
    label: `Department (${emp.department}) — ${deptGL}`, 
    value: deptGL 
  });
}
...
const add = $('#addRowBtn');        
if (add) add.addEventListener('click', () => addTimeRow());
const clr = $('#clearAllBtn');      
if (clr) clr.addEventListener('click', clearAllRows);
const th  = $('#townHallBtn');      
if (th)  th.addEventListener('click', addTownHallHours);
const form= $('#timesheetForm');    
if (form)form.addEventListener('submit', handleSubmit);

// Supplemental action buttons
const split = $('#splitShiftBtn');  
if (split) split.addEventListener('click', addSplitShift);
const ot    = $('#overtimeBtn');    
if (ot)    ot.addEventListener('click', addOvertimeDay);
const sick  = $('#sickBtn');        
if (sick)  sick.addEventListener('click', addSickDay);
const vac   = $('#vacationBtn');    
if (vac)   vac.addEventListener('click', addVacationDay);
const pers  = $('#personalBtn');    
if (pers)  pers.addEventListener('click', () => addQuickDay('Personal', 'Personal Day', '8.00'));
const hol   = $('#holidayBtn');     
if (hol)   hol.addEventListener('click', () => addQuickDay('Holiday', 'Holiday', '8.00'));

// Enter key advances through table fields
document.addEventListener('keydown', (ev) => {
  if (ev.key === 'Enter' && ev.target && ev.target.matches && 
      ev.target.matches('#timeBody input, #timeBody select')) {
    ev.preventDefault();

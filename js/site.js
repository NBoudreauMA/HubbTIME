...
window.HubbTIME = (function () {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // Read response JSON & surface prefill URL if the flow returns it
      let data = null;
      try { data = await res.json(); } catch (e) {}
      if (data && data.prefillUrl) {
        if (status) {
          status.innerHTML = `Submitted ✔ · <a href="${data.prefillUrl}" target="_blank" rel="noopener">Open in CO</a>`;
        }
      } else {
